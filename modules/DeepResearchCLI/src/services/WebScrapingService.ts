...
export class WebScrapingService {
  ...
  async scrapePage(url: string, depth: number = 0): Promise<PageData> {
    ...
    try {
      ...
      if (docType) {
        console.log(
          `    ${docType.toUpperCase()} file detected, downloading and extracting content...`
        );
      ...
      // Detect if page uses JavaScript frameworks (AFTER navigation)
      const isJavaScriptApp = await this.detectJavaScriptApp(page, url);

      // if (isJavaScriptApp.detected) {
      //   console.log(
      //     `   JavaScript framework detected (${isJavaScriptApp.confidence} confidence): ${isJavaScriptApp.frameworks.join(", ")}`
      //   );
      //   console.log(`   ⏳ Waiting for dynamic content to load...`);
        
      //   // Wait longer for JS-heavy apps
      //   // await page.waitForTimeout(5000);
      // } else {
      //   // Standard wait for non-JS apps
      ...
      if (jsCheck.isJSRendered) {
        console.log(
          `    JavaScript-rendered page confirmed: ${jsCheck.indicators.join(", ")}`
        );
      }
      ...
      if (textContent.length < 100) {
        console.log(`    WARNING: Very short content extracted!`);
        if (jsCheck.isJSRendered) {
          console.log(
            `   This is a JS-rendered page - content may be loading dynamically`
          );
          await page.waitForTimeout(3000);
          await this.handleDynamicContent(page);
      ...
      console.log(`    Failed after: ${fetchTime}ms`);
  ...
  private extractContentAndLinks(
    ...
    if (!primaryContent) {
      console.log(
        `        No content found with standard selectors, trying fallback...`
      );
      ...
      if (paragraphs.length > 200) {
        ...
        console.log(
          `       Found content from paragraphs (${paragraphs.length} chars)`
        );
      } else {
        primaryContent = $("body").text().trim();
        console.log(
          `        Using body text as last resort (${primaryContent.length} chars)`
        );
      }
    }
    ...
    console.log(
      `      Combined content: ${fullContent.length} chars (before cleanup)`
    );
    ...
    console.log(
      `      Final content: ${textContent.length} chars (after cleanup, limit 15000)`
    );
    ...
    console.log(
      `       Valid unique links extracted: ${links.length} (limited to 50)`
    );
  ...
  /**
   * Detect if the page is a JavaScript-based application by analyzing the initial HTML
   * This is called AFTER navigation to inspect actual page content
   */
  private async detectJavaScriptApp(
    page: Page,
    url: string
  ): Promise<{ detected: boolean; frameworks: string[]; confidence: 'high' | 'medium' | 'low' }> {
    try {
      const frameworks: string[] = [];
      let confidence: 'high' | 'medium' | 'low' = 'low';

      // Get initial HTML content to analyze
      ...
      const detectionResults = await page.evaluate(() => {
        ...
        // React detection
        results.hasReact = !!(window as any).React || 
                          !!document.querySelector('[data-reactroot]') ||
                          !!document.querySelector('[data-reactid]') ||
                          !!document.querySelector('[data-react-helmet]');

        // Vue detection
        results.hasVue = !!(window as any).Vue ||
                        !!document.querySelector('[data-v-]') ||
                        !!document.querySelector('[data-server-rendered]');

        // Angular detection
        results.hasAngular = !!(window as any).angular ||
                            !!(window as any).ng ||
        ...
        // Next.js detection
        results.hasNext = !!(window as any).__NEXT_DATA__ ||
                         !!document.getElementById('__next');

        // Nuxt.js detection
        results.hasNuxt = !!(window as any).__NUXT__ ||
                         !!document.getElementById('__nuxt');

        // Svelte detection
        results.hasSvelte = !!document.querySelector('[class*="svelte-"]');

        // Ember detection
        results.hasEmber = !!(window as any).Ember ||
                          !!document.querySelector('[class*="ember-"]');
        ...
        if (appElement) {
          results.rootElementId = appElement.id;
        }

        // Count script tags (SPAs typically have many)
        results.scriptCount = document.querySelectorAll('script').length;

        // Check if body has minimal server-rendered content
        const bodyText = document.body.innerText.trim();
        results.hasMinimalContent = bodyText.length < 200;

        return results;
      });

      // Analyze HTML for framework signatures
      const htmlPatterns = {
        react: /react|reactdom|_react/i,
        vue: /vue\.js|vuejs|_vue/i,
        angular: /angular|ng-|@angular/i,
        next: /next\.js|_next/i,
        nuxt: /nuxt\.js|_nuxt/i,
        svelte: /svelte/i,
        ember: /ember/i
      };

      // Framework detection with confidence levels
      if (detectionResults.hasReact || htmlPatterns.react.test(htmlLower)) {
        frameworks.push('React');
        confidence = detectionResults.hasReact ? 'high' : 'medium';
      }

      if (detectionResults.hasNext || htmlPatterns.next.test(htmlLower)) {
        frameworks.push('Next.js');
        confidence = 'high';
      }

      if (detectionResults.hasVue || htmlPatterns.vue.test(htmlLower)) {
        frameworks.push('Vue');
        confidence = detectionResults.hasVue ? 'high' : 'medium';
      }

      if (detectionResults.hasNuxt || htmlPatterns.nuxt.test(htmlLower)) {
        frameworks.push('Nuxt.js');
        confidence = 'high';
      }

      if (detectionResults.hasAngular || htmlPatterns.angular.test(htmlLower)) {
        frameworks.push('Angular');
        confidence = detectionResults.hasAngular ? 'high' : 'medium';
      }

      if (detectionResults.hasSvelte || htmlPatterns.svelte.test(htmlLower)) {
        frameworks.push('Svelte');
        confidence = detectionResults.hasSvelte ? 'high' : 'medium';
      }

      if (detectionResults.hasEmber || htmlPatterns.ember.test(htmlLower)) {
        frameworks.push('Ember');
        confidence = detectionResults.hasEmber ? 'high' : 'medium';
      }

      // Additional heuristics for SPA detection
      const isSPA = detectionResults.scriptCount > 5 && 
                    detectionResults.hasMinimalContent &&
                    !!detectionResults.rootElementId;

      if (isSPA && frameworks.length === 0) {
        frameworks.push('Unknown SPA');
        confidence = 'low';
      }
      ...
      return {
        detected: uniqueFrameworks.length > 0,
        frameworks: uniqueFrameworks,
        confidence
      };
    } catch (error) {
      // If detection fails, assume it's not a JS app
      return { detected: false, frameworks: [], confidence: 'low' };
    }
  }
   ...
   */
  private async checkJavaScriptRendering(
    page: Page
  ): Promise<{ isJSRendered: boolean; indicators: string[] }> {
    try {
   ...
   */
  private async scrapeTextDocument(
    url: string,
    depth: number,
    docType: string
  ): Promise<PageData> {
    ...
    try {
      ...
      console.log(
        `   📥 Downloaded ${docType.toUpperCase()}: ${buffer.length} bytes`
      );
      ...
      console.error(
        `   ❌ Failed to extract ${docType.toUpperCase()}: ${error}`
      );
      throw error;
    }
  }
