...
```rust
impl ConnectorIntegrationV2<Flow, ResourceData, RequestData, ResponseData> for {{connector_name}} {
    fn get_headers(
        &self,
...
<output_file>
Store the result in grace/connector_integration/{{connector_name}}/{{connector_name}}_specs.md
</output_file>
...
```rust
// UCS uses RouterDataV2 for all operations (4 type params: Flow, ResourceData, RequestData, ResponseData)
type AuthorizeRouterData = RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>;
