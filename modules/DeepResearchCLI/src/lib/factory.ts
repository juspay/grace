import { MassResearcher } from './MassResearcher';
import { MassConfig } from './MassConfig';
import { readFileSync, existsSync } from 'fs';
import { resolve } from 'path';

/**
 * Create a default configuration for MASS researcher
 */
export function createDefaultConfig(): MassConfig {
  return {
    ai: {
      provider: 'litellm',
      modelId: 'gpt-4',
...
/**
 * Create a MassResearcher instance with smart configuration detection
 *
 * This function will:
 ...
 */
export function createMassResearcher(config: Partial<MassConfig> = {}): MassResearcher {
  const defaultConfig = createDefaultConfig();
  const envConfig = loadConfigFromEnvironment();

  // Merge configurations: defaults <- env <- provided
  const finalConfig: MassConfig = {
    ai: { ...defaultConfig.ai, ...envConfig.ai, ...config.ai },
    research: { ...defaultConfig.research, ...envConfig.research, ...config.research }
  };

  return new MassResearcher(finalConfig);
}

/**
 * Create a MassResearcher from environment variables and .env file
 */
export function createMassResearcherFromEnv(envPath?: string): MassResearcher {
 ...
 */
function loadConfigFromEnvironment(envPath?: string): MassConfig {
 ...
 */
export function validateConfig(config: MassConfig): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
