...
14. **Scoped git staging** — when committing on the dev branch (Phase 1), only stage connector-specific files (`git add backend/connector-integration/src/connectors/{connector}*`). Never `git add -A`. Never stage `creds.json` or `.env`.
