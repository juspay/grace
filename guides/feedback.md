...
let auth = ConnectorSpecificConfig::{{ConnectorName}} {
    api_key: router_data.connector_config.api_key.clone(),
    key1: router_data.connector_config.key1.clone(),
    ..
};
```
...
2. Determine correct `ConnectorSpecificConfig` variant:
   - `{{ConnectorName}} { api_key, .. }` - API key in header
   - `{{ConnectorName}} { api_key, key1, .. }` - Multiple keys
   - `{{ConnectorName}} { ... }` - Signature-based auth
3. Extract auth from `router_data.connector_config`
...
IF ConnectorSpecificConfig has unused fields
