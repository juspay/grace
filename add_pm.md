...
The subagent CANNOT and WILL NOT read the `add_pm.md` file directly.
...
```bash
add {Category}:{type1},{type2} and {Category2}:{type3} to {connector_name} using grace/add_pm.md
```
...
Completed:
[DONE] {PaymentMethod1} - Successfully added
[DONE] {PaymentMethod2} - Successfully added
```
...
5. FINAL VERIFICATION:
   [DONE] All 3 payment methods added successfully
   [DONE] Cargo build passes
   [DONE] No breaking changes to existing Card support
```
...
# Suggest: "First add Authorize flow using add_flow.md"
```
