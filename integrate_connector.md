...
- User has placed tech spec in grace/references/{connector_name}/technical_specification.md
...
6. **MANDATORY**: Detect pre-authorization flows required by the connector from tech spec:
   - **CreateAccessToken**: Connector uses OAuth/token-based auth (e.g., POST /login, POST /oauth/token)
   ...
   - **CreateSessionToken**: Connector requires session initialization before payment
...
- IF CreateAccessToken needed: **DELEGATE TO**: CreateAccessToken Flow Subagent → **WAIT FOR COMPLETION**
...
- IF CreateSessionToken needed: **DELEGATE TO**: CreateSessionToken Flow Subagent → **WAIT FOR COMPLETION**
...
When user requests: "integrate {ConnectorName} using grace/integrate_connector.md"
...
```
STEP 1: Read tech spec from grace/references/{connector_name}/technical_specification.md
...
#    Find the enum definition and add a new variant:
#    {ConnectorName} {
#        api_key: Secret<String>,       // Adjust fields based on tech spec auth requirements
#        base_url: Option<String>,      // ALWAYS include base_url for runtime override
#    },
#    NOTE: Use Secret<String> for all credential fields. Field names should be semantically
#    meaningful (e.g., api_key, api_secret, merchant_id, not key1/key2).

# 2. ADD TO base_url_override MACRO (~line 670-950):
#    Find the extract_base_url! macro invocation and add:
#    {ConnectorName} { api_key },
#    NOTE: List ALL fields EXCEPT base_url (the macro extracts base_url automatically via `..`)

# 3. ADD TO connector_key MACRO (~line 1039-1100):
#    Find the connector_key! macro invocation and add:
#    {ConnectorName} { api_key },
#    NOTE: Same field list as base_url_override (all fields except base_url)

# 4. ADD ForeignTryFrom MATCH ARM (~line 1700-2000 in the ForeignTryFrom impl):
#    Find the ForeignTryFrom<(ConnectorEnum, &ConnectorAuthType)> impl and add:
#    ConnectorEnum::{ConnectorName} => match auth {
#        ConnectorAuthType::HeaderKey { api_key } => Ok(Self::{ConnectorName} {
#            api_key: api_key.clone(),
#            base_url: None,
#        }),
#        _ => Err(err().into()),
#    },
#    NOTE: Match the ConnectorAuthType variant to your connector's auth pattern:
#    - HeaderKey { api_key } for single API key (Bearer token, etc.)
#    - BodyKey { api_key, key1 } for API key + secret
#    - SignatureKey { api_key, api_secret, key1 } for 3-field auth

# 5. ADD TO dummy_auth() in backend/field-probe/src/main.rs:
#    Find the dummy_auth() function and add:
#    ConnectorEnum::{ConnectorName} => ConnectorSpecificConfig::{ConnectorName} {
#        api_key: Secret::new("dummy".into()),
#        base_url: None,
#    },

# 6. ADD TO all_connectors() in backend/field-probe/src/main.rs:
#    Find the all_connectors() function and add:
...
- **Pattern File**: guides/patterns/pattern_CreateAccessToken_flow.md
- **Primary Responsibility**: OAuth/token acquisition before payment flows
- **Key Implementation**: ConnectorIntegrationV2<CreateAccessToken, PaymentFlowData, AccessTokenRequestData, AccessTokenResponseData>
...
- **Pattern File**: (no dedicated pattern — follow macro patterns and reference connectors like Stripe,Shift4)
...
- **Flow Name**: CreateSessionToken
- **Pattern File**: guides/patterns/pattern_session_token.md
- **Primary Responsibility**: Session initialization before payment
- **Key Implementation**: ConnectorIntegrationV2<CreateSessionToken, PaymentFlowData, SessionTokenRequestData, SessionTokenResponseData>
...
4. Assess Code Quality (Entire Codebase)
   ...
   **ERROR HANDLING QUALITY CHECKS:**
   ...
   - Proper ConnectorError types used throughout
...
8. Quality Metrics checklist
   - Mark each compliance item as PASS or FAIL
...
60-79:  Fair - Pass with warnings, recommend fixes
...
# ============================================================================
# ERROR HANDLING AND UCS COMPLIANCE
# ============================================================================
...
- Tech spec file exists in grace/references/{connector_name}/technical_specification.md
...
When user executes: "integrate {ConnectorName} using grace/integrate_connector.md"
...
- "integrate {ConnectorName} connector using grace/integrate_connector.md"
