...
### Unsupported Payment Method Errors

**IMPORTANT:** When a payment method is not supported by the connector, always return a specific, descriptive error message.
...
match &router_data.request.payment_method_data {
    ...
    _ => return Err(ConnectorError::NotSupported {
        message: "Payment method not supported".to_string(), // Too generic!
    }.into()),
}
```

#### ✅ RIGHT: Specific Error Message
```rust
use domain_types::{errors::ConnectorError, payment_method_data::PaymentMethodData};

// CORRECT: Specific error with payment method details
impl<T: PaymentMethodDataTypes + std::fmt::Debug + std::marker::Sync + std::marker::Send + 'static + Serialize>
    TryFrom<{ConnectorName}RouterData<RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>, T>>
    for {ConnectorName}AuthorizeRequest<T>
{
    ...
    fn try_from(
        ...
        // Extract payment method with specific error messaging
        let payment_method = match &router_data.request.payment_method_data {
            PaymentMethodData::Card(card_data) => {
                // Handle card payment
                {ConnectorName}PaymentMethod::Card({ConnectorName}Card {
            ...
            PaymentMethodData::Wallet(wallet_data) => {
                return Err(ConnectorError::NotSupported {
                    message: format!("Payment method {} is not supported", "Wallet"),
                    connector: "{ConnectorName}",
                    payment_method_type: wallet_data.to_string(),
                }.into())
            },
            PaymentMethodData::BankTransfer(_) => {
                return Err(ConnectorError::NotSupported {
                    message: "Payment method BankTransfer is not supported".to_string(),
                    connector: "{ConnectorName}",
            ...
            payment_method => {
                return Err(ConnectorError::NotSupported {
                    ...
                    connector: "{ConnectorName}",
                    payment_method_type: format!("{:?}", payment_method),
                }.into())
            },
        };
...
**Key Points:**
- ✅ Use `ConnectorError::NotSupported` for unsupported payment methods
- ✅ Include specific payment method name in the error message
- ✅ Format: `"Payment method {method} is not supported"`
- ❌ Never use generic messages like "Payment method not supported"
