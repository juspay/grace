...
export class ResultOutputService {
  ...
  async saveResults(session: ResearchSession, allPages: PageData[]): Promise<{
    htmlPath: string;
    jsonPath: string;
    markdownPath: string;
  }> {
    ...
    const baseName = `research_${timestamp}`;
  ...
  private generateMarkdownReport(session: ResearchSession, pages: PageData[]): string {
    ...
    const averageRelevance = successfulPages.length > 0 ?
      successfulPages.reduce((sum, p) => sum + p.relevanceScore, 0) / successfulPages.length : 0;
    ...
    Array.from(sourcesByDepth.keys()).sort().forEach(depth => {
      const depthPages = sourcesByDepth.get(depth)!
      ...
      depthPages.forEach((page, index) => {
        ...
        const relevanceIcon = page.relevanceScore >= 0.7 ? '🟢' : page.relevanceScore >= 0.4 ? '🟡' : '🔴';
  ...
  private generateHTMLReport(session: ResearchSession, pages: PageData[]): string {
    const duration = session.endTime ? (session.endTime - session.startTime) / 1000 : 0;
    const successfulPages = pages.filter(p => !p.error);
...
<body>
    ...
    <div style="text-align: center; margin-top: 30px; color: #6b7280; font-size: 0.9em;">
        <p>Generated on ${new Date().toLocaleString()} by MASS Deep Research CLI v1.0.0</p>
    </div>
  ...
  private generateSourcesHTML(pages: PageData[]): string {
    ...
    Array.from(sourcesByDepth.keys()).sort().forEach(depth => {
      ...
      depthPages.forEach((page, index) => {
        const relevanceClass = page.relevanceScore >= 0.7 ? 'relevance-high' :
          page.relevanceScore >= 0.4 ? 'relevance-medium' : 'relevance-low';
  ...
  public async openResult(htmlPath: string): Promise<void> {
    ...
    console.log(chalk.cyan(`   JSON Data: ${htmlPath.replace('.html', '.json')}`));
