...
class MassResearchCLI {
  ...
  async startDirectResearch(query: string): Promise<void> {
    try {
      console.log(chalk.cyan.bold('🚀 MASS Direct Research Mode'));
      ...
      console.error(chalk.red('\n❌ Direct research failed:'), error instanceof Error ? error.message : 'Unknown error');
      process.exit(1);
    }
  }
