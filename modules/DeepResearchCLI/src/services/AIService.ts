...
export class AIService {
  ...
  private enhanceMessagesWithCustomInstructions(
    messages: AIMessage[]
  ): AIMessage[] {
  ...
  async generateText(
    ...
    try {
      ...
      const enhancedMessages =
        this.enhanceMessagesWithCustomInstructions(messages);
      ...
      const testMessages: AIMessage[] = [
        {
          role: "user",
          content:
            'Reply with exactly "TEST_OK" if you can process this message.',
        },
      ];
...
`;
  ...
  private async callVertexAI(
    ...
    try {
      ...
      if (!projectId) {
        throw new Error(
          "Project ID not found. Set VERTEX_AI_PROJECT_ID or configure gcloud CLI."
        );
      }
      ...
      if (modelId.includes("claude") || modelId.includes("anthropic")) {
        return await this.callVertexAnthropicSDK(
          projectId,
          location,
          modelId,
          messages,
        ...
        return await this.callVertexGemini(
          projectId,
          location,
          modelId,
          messages,
...
Error details: ${error.message}`
    ...
    const messages: AIMessage[] = [
      {
        ...
        content: `You are a relevance scoring expert. Rate how relevant the given content is to the query on a scale of 0.0 to 1.0. Respond with only a number. 
          ignore social media links and programming language docs links except related to the custom instructions topics,
          also you have to remember that relevance should be related to this custom instruction topics customInstructions : ${this.config.customInstructions || "N/A"}`,
      },
      {
        role: "user",
    ...
    const messages: AIMessage[] = [
      {
        role: "system",
        content:
          "Extract 3-5 key insights from the content that are relevant to the query. Return as a JSON array of strings.",
      },
      {
        role: "user",
  ...
  async generateSearchQueries(
    ...
    let systemPrompt = `Generate 2-5 search queries to find comprehensive information about the topic.
  ...
  async analyzeSearchResults(
    ...
    //   .slice(0, 5)
    //   .map((res, idx) => `${idx + 1}. ${res.title} - ${res.snippet}`)
    //   .join("\n");
    let systemPrompt = `You are a search results analyzer. 
    ...
    Top Search Results: ${results
      .map(
        (res, idx) =>
          `${idx + 1}. ${res.title} - ${res.snippet} (url : ${res.url})`
      )
      .join("\n")}

    Return as a JSON array of relavent url links from provided search results.
    ...
    const messages: AIMessage[] = [
      ...
      {
        role: "user",
        content: `Analyze the search results for the query: "${query}"\n\nProvide a summary of key findings: 
        Return as a JSON array of relavent url links from provided search results.
        ["url1", "url2"] from the given list of search results.`,
      },
    ];
    ...
    try {
      ...
      console.warn("Failed to analyze search results:", error);
      return [];
    }
  }
...
Should we continue to depth ${currentDepth + 1}?`,
    ...
    try {
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI evaluating whether to continue crawling (depth ${currentDepth}/${maxDepth})`
        );
      }
      ...
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI decision: ${result.shouldContinue ? "CONTINUE" : "STOP"} - ${result.reason}`
        );
      }
      ...
      console.warn(
        "Failed to get AI crawling decision, defaulting to continue:",
        error
      );
      return {
  ...
  async rankLinksForCrawling(
    ...
    const messages: AIMessage[] = [
      {
        role: "system",
        content: `You are a research url link evaluator. 
...
However, prioritize HTML pages when both have similar relevance, as they typically have better structure.`,
      ...
      {
        role: "user",
        content: `Custom Instructions: ${this.config.customInstructions || "N/A"}`,
      },
      {
        role: "user",
        content: `Research Query: "${query}"
...
Rank these links`,
    ...
    try {
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI ranking ${linksToEvaluate.length} links for crawling`
        );
      }
      ...
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI ranked ${rankedLinks.length} links, top score: ${rankedLinks[0]?.score || 0}`
        );
      }
      ...
      console.warn(
        "Failed to rank links with AI, using default scoring:",
        error
      );
  ...
  async assessInformationCompleteness(
    ...
    confidence: number;
    customInstructionsMet?: boolean;
  }> {
    const hasCustomInstructions =
      this.config.customInstructions &&
      this.config.customInstructions.trim().length > 0;
...
Respond in JSON:
{
  ...
  "confidence": 0.0-1.0${hasCustomInstructions ? ',\n  "customInstructionsMet": true/false,\n  "missingRequirements": ["requirement1"]' : ""}
    ...
    const messages: AIMessage[] = [
      ...
      {
        role: "assistant",
        content: `Pages Collected: ${pagesCollected}`,
      },
      ...insights.map(
        (insight, idx): AIMessage => ({
          role: "assistant",
          content: `Source ${idx + 1}: ${insight}`,
        })
      ),
      {
        role: "user",
    ...
    try {
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI assessing information completeness (custom instructions: ${hasCustomInstructions})`
        );
      }
      ...
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `Completeness: ${result.isComplete}, Custom met: ${result.customInstructionsMet ?? "N/A"}`
        );
      ...
      return {
        ...
        missingAspects: Array.isArray(result.missingAspects)
          ...
          : [],
        confidence: Math.max(0, Math.min(1, result.confidence || 0.5)),
        customInstructionsMet: hasCustomInstructions
          ? (result.customInstructionsMet ?? false)
          : undefined,
      };
    } catch (error) {
  ...
  async synthesizeResults(
  ...
  ): Promise<{ answer: string; confidence: number; summary: string }> {
    ...
    //   .map(
    //     (item, index) =>
    //       `[${index + 1}] ${item.title} (Relevance: ${item.relevanceScore.toFixed(2)}, Depth: ${item.depth})\nURL: ${item.url}\nContent: ${item.content}...`
    //   )
    //   .join("\n\n---\n\n");
    ...
    let parseResponse: (content: string) => {
      answer: string;
      confidence: number;
      summary: string;
    };
    ...
    if (hasCustomInstructions) {
      ...
      systemPrompt = `
You are an expert research analyst. Based on the collected research data, provide a comprehensive analysis following the custom format requirements that will be provided.
also you have to give the confidence which will satisfy this regex : '/confidence[:\s]+([0-9.]+)/i'
...
`;
      ...
      parseResponse = (content: string) => {
        try {
          ...
          const summary = summaryMatch
            ? summaryMatch[1].trim()
            : "Comprehensive analysis completed following custom instructions.";
      ...
      systemPrompt = `
You are an expert research analyst. Provide a comprehensive analysis based on the collected research data.
...
`;
      ...
      parseResponse = (content: string) => {
        try {
          if (this.debugLogger.isEnabled()) {
            ...
            this.debugLogger.log(
              `  Raw content length: ${content.length} chars`
            );
          }
          ...
          if (this.debugLogger.isEnabled()) {
            this.debugLogger.log(
              `  Cleaned content length: ${cleanedContent.length} chars`
            );
            this.debugLogger.log(
              `  First 200 chars: ${cleanedContent.substring(0, 200)}`
            );
          }
    ...
    const messages: AIMessage[] = [
      ...
      ...sortedContent.map(
        (item, index): AIMessage => ({
          role: "user",
          content: `Source: ${index + 1} Title: ${item.title}\nURL: ${item.url}\nContent: ${item.content}...`,
        })
      ),
      {
        role: "user",
        content: `Research Query: "${query}"\n\nCollected Data from ${sortedContent.length} \n\n Provide comprehensive analysis following the custom instructions provided earlier.`,
      },
    ];
    ...
    try {
      if (this.debugLogger.isEnabled()) {
        ...
        this.debugLogger.log(
          `Custom instructions: ${hasCustomInstructions ? "ENABLED" : "DISABLED"}`
        );
      }
      ...
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI response received: ${response.content.length} chars`
        );
      ...
      if (this.debugLogger.isEnabled()) {
        ...
        this.debugLogger.log(
          `Answer length: ${parsedResult.answer.length} chars`
        );
   ...
   */
  async generateLinkQualityPrompt(
...
Your output should be a single paragraph (2-6 sentences) that captures the CORE criteria for link quality.`,
      },
      {
        role: "user",
        content: `${query ? `Research Query: "${query}"\n\n` : ""}Custom Instructions:\n\n${customInstructions}\n\n Generate a concise prompt for evaluating if a link is valuable for this research:`,
      },
    ];
  ...
  > {
    ...
    const linksList = linksToEvaluate
      .map(
        (link, idx) =>
          `${idx + 1}. [${link.text}](${link.url})\nContext: ${link.context?.substring(0, 200) || "N/A"}`
      )
      .join("\n\n");
...
remember to follow the instructions provided by the user in the custom instructions.
...
Quality levels:
- high: Directly valuable, official docs, technical content from the source
- medium: Potentially useful, needs verification
- low: Skip this - marketing, unrelated, or low value

IMPORTANT: PDFs (.pdf) can be evaluated normally if they contain relevant information.

NOTE : DON'T RETURN THE LINKS WHICH ARE NOT RELEVANT TO THE CUSTOM INSTRUCTIONS.
...
${this.config.customInstructions || "N/A"}`,
      },
      {
        role: "user",
        content: `Research Query: "${query}"
...
Evaluate each link's quality:`,
    ...
    try {
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `AI evaluating quality of ${linksToEvaluate.length} links`
        );
      }
      ...
      const evaluatedLinks = linksToEvaluate.map((link, idx) => {
        ...
        return {
          ...link,
          quality: (evaluation?.quality || "medium") as
            | "high"
            | "medium"
            | "low",
      ...
      const filteredLinks = evaluatedLinks.filter(
        (link) => link.quality !== "low"
      );

      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `Link quality filtering: ${linksToEvaluate.length} -> ${filteredLinks.length} (removed ${linksToEvaluate.length - filteredLinks.length} low-quality links)`
        );
        this.debugLogger.log(
          `High quality: ${filteredLinks.filter((l) => l.quality === "high").length}, Medium: ${filteredLinks.filter((l) => l.quality === "medium").length}`
        );
      }
      ...
      if (this.debugLogger.isEnabled()) {
        this.debugLogger.log(
          `Link quality filtering failed: ${error}. Returning all links.`
        );
      }
  ...
  async generateNamedDescription( query: string): Promise<string> {
    const messages: AIMessage[] = [
        {
          role: "system",
          content: `You are a Name generator for the user query 
          which should be a single line specific named related to the content or query provided by the user.
          The name should be unique, relevant to the content, and easy to remember.
          The name should not contain special characters or spaces. Use hyphens (-) or underscores (_) if needed.
          based on the query: ${query}
          `,
        },
      ];
      const name = await this.generateText(messages, {
        temperature: 0.1,
        maxTokens: 10,
      });
      return name.content.replace(/[^a-zA-Z0-9-_ ]/g, '').trim().replace(/\s+/g, '-').toLowerCase();
    }
}
