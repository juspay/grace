...
macros::macro_connector_implementation!(
    connector = {ConnectorName}<T>,
    flow = RSync,
    resource_data = RefundFlowData,
    request_data = RefundSyncData,
    response_data = RefundsResponseData,
    http_method = Method::Get,
    url = |this: &Self, req| {
        let refund_id = req.request.connector_refund_id.clone();
        Ok(format!("{}/refunds/{}", this.connector_base_url_refunds(req), refund_id))
    },
    body = |_this: &Self, _req, _connectors| {
        Ok(RequestContent::Json(Box::new(serde_json::Value::Null)))
    }
);
```
...
mod rsync_tests {
    use super::*;
    use crate::connector_auth::ConnectorSpecificConfig;
