...
class GraceResearchCLI {
  ...
  async startInteractiveResearch(): Promise<void> {
    try {
      ...
      const { AIService, AIConfigurationError } = await import(
        "../services/AIService"
      );
        ...
        const { continueAnyway } = await inquirer.prompt([
          {
            ...
            name: "continueAnyway",
            message:
              "Would you like to continue anyway? (Research will fail without AI)",
            default: false,
          },
        ]);
      ...
      const { query } = await inquirer.prompt([
        {
          ...
          validate: (input: string) =>
            input.trim().length > 0 || "Please enter a research query",
        },
      ]);
      let customInstructions =
        this.config.getAIConfig().customInstructions || "";
      // Get optional custom instructions
      if (!this.config.getAIConfig().customInstructions) {
        const { customInstructions: instructions } = await inquirer.prompt([
          {
            type: "input",
      ...
      const researchConfig = this.config.getResearchConfig();
      let maxDepth = 5;
      let maxPagesPerDepth = 5;
      if (!this.config.getResearchConfig().maxDepth) {
        maxDepth = await inquirer.prompt([
          {
            type: "number",
            name: "maxDepth",
            ...
            validate: (input: number) =>
              (input >= 1 && input <= 10) || "Depth must be between 1 and 10",
          },
        ]);
      }
      if (!this.config.getResearchConfig().maxPagesPerDepth) {
        maxPagesPerDepth = await inquirer.prompt([
          {
            type: "number",
            ...
            validate: (input: number) =>
              (input >= 1 && input <= 50) || "Pages must be between 1 and 50",
          },
        ]);
      }
      // Initialize UI
      this.ui = new TerminalUI({
        title: "GRACE - Deep Research CLI",
    ...
    switch (action.type) {
      ...
      case "input":
        if (action.data?.message) {
          ...
          this.ui?.updateStatus(
            `Guidance provided: "${action.data.message}"`,
            "info"
          );
        }
        break;
  ...
  private async handleResearchComplete(): Promise<void> {
    ...
    this.ui?.updateStatus(
      "Research completed! Generating reports...",
      "success"
    );
    ...
    try {
      ...
      // Save results
      const name = await this.orchestrator.generateNameForSession(
        this.currentSession.query
      );
      const { htmlPath, jsonPath, markdownPath } =
        await this.resultOutputService.saveResults(
          this.currentSession,
          name,
          allPages
        );
      ...
      if (this.ui) {
        ...
        setTimeout(async () => {
          this.cleanup();
          if (htmlPath) await this.resultOutputService.openResult(htmlPath);
          ...
          console.log(
            chalk.white(`Pages Processed: ${this.currentSession!.totalPages}`)
          );
          console.log(
            chalk.white(`Max Depth: ${this.currentSession!.maxDepthReached}`)
          );

          if (this.currentSession!.confidence !== undefined) {
            console.log(
              chalk.white(
                `Confidence: ${(this.currentSession!.confidence * 100).toFixed(1)}%`
              )
            );
          }
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log(
        chalk.green(`Cleanup complete! Removed ${deletedCount} old sessions.`)
      );
    } catch (error) {
      console.error(
