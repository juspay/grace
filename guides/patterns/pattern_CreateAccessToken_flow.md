...
#### Request Data
```rust
// AccessTokenRequestData - Empty struct, auth details come from connector_config
...
impl TryFrom<&ConnectorSpecificConfig> for VoltAuthType {
    ...
    fn try_from(auth_type: &ConnectorSpecificConfig) -> Result<Self, Self::Error> {
        if let ConnectorSpecificConfig::{{ConnectorName}} {
            api_key,
            key1,
            api_secret,
            key2,
...
pub fn get_headers(
    ...
    let auth = ConnectorSpecificConfig::try_from(&req.connector_config)?;
...
mod tests {
    ...
    fn test_auth_update_request_conversion() {
        let auth_type = ConnectorSpecificConfig::{{ConnectorName}} {
...
1. **Identify Auth Type**: Determine which `ConnectorSpecificConfig` variant holds OAuth credentials
   - `ConnectorSpecificConfig::{{ConnectorName}} { api_key, key1, .. }` - Common for client_id/client_secret
   - `ConnectorSpecificConfig::{{ConnectorName}} { api_key, key1, api_secret, key2, .. }` - For 4-field auth
...
match auth_type {
    ConnectorSpecificConfig::{{ConnectorName}} { .. } => Ok(...),
